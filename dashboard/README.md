...
| `dashboard_config.example.json` | Example configuration for a later step. |
| `tests/test_submission.py` | `unittest` tests for both modules. |
| `.gitignore` | Keeps the demo workspace, the real config and bytecode out of git. |
| `README.md` | This file. |

`API_EXAMPLE/`, `final_output/` and `snapshots/` are inherited references and
are left untouched.
...
will be written. Both paths are relative to the configuration file itself, not
to the current working directory. A later step will copy this file to
`dashboard_config.json`, resolve the paths against the config file's directory
and create the directories. Nothing in steps 1 and 2 reads this file.
...
Use `python` instead of `python3` if that is the interpreter on your PATH,
which is the usual case on Windows.

`python3 -m unittest` puts the current directory on `sys.path`, so
`apr_dashboard` imports without any packaging step. There is deliberately no
`tests/__init__.py`, no `setup.py` and no `pyproject.toml`. The metadata tests
build their run trees under a temporary directory, so they need no fixtures and
